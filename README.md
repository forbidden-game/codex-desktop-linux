...
| GPU/Vulkan/Wayland errors | The launcher defaults to X11 on GNOME Wayland for more reliable Fcitx candidate positioning, and otherwise uses `--ozone-platform-hint=auto` with Wayland IME enabled. Override with `CODEX_DESKTOP_OZONE_PLATFORM=x11|wayland|auto ./codex-app/start.sh` |
| Fcitx candidate window is offset from the cursor | Restart the app after updating. On GNOME Wayland the launcher should start Electron with `--ozone-platform=x11`; use `ps -eo pid,args \| grep /opt/codex-desktop/electron` to confirm |
